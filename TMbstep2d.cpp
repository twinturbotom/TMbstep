...
//const T os    = 5.0;    // origin of step
const T lx0   = 2.2;//20.0;   // length of channel in meter
...
void prepareGeometry( LBconverter<T> const& converter,
  ...
  Vector<T,2> originStep( os, 0.0);//originStep;
...
int main( int argc, char* argv[] ) {
  float os = atof(argv[1]);
  float lx1 = atof(argv[2]);
  float ly1 = atof(argv[3]);
...
/*  if (argc!=2) {
  ...
  olbInit( &argc, &argv );
  singleton::directories().setOutputDir( fName );  // set output directory
  OstreamManager clout( std::cout, "main" );

  LBconverter<T> converter(
    ( int ) 2,                             // dim
    ( T )   0.01,//1./60./N,                      // latticeL_
    ( T )   0.01,//2e-2/M,                        // latticeU_
    ( T )   0.1,//1./500.,                       // charNu_
    ( T )   0.2//1.                             // charL_ = 1,
  );
  converter.print();
  ...
  prepareGeometry( converter, superGeometry, os, lx1, ly1 );
